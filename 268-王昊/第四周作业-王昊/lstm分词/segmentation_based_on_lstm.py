...
def main(vocab_path, corpus_path):
    epoch_num = 10        #训练轮数
    ...
    for epoch in range(epoch_num):
        ...
        for x, y in pbar:
            ...
            pbar.set_description('Epoch:%d/%d loss: %.5f' % (epoch + 1, epoch_num, loss.item()))
        print("=========\n第%d轮平均loss:%f" % (epoch + 1, np.mean(watch_loss)))
    #保存模型
    torch.save(model.state_dict(), "./lstm分词/模型/model.pth")
    return
...
if __name__ == "__main__":
    vocab_path = os.path.abspath("./lstm分词/chars.txt")  #字表文件路径
    corpus_path = os.path.abspath("./corpus/corpus.txt")  #语料文件路径
    model_path = os.path.abspath("./lstm分词/模型/model.pth")  # 模型路径

    main(vocab_path, corpus_path)
    input_strings = ["同时国内有望出台新汽车刺激方案",
                     "沪胶后市有望延续强势",
                     "经过两个交易日的强势调整后",
                     "昨日上海天然橡胶期货价格再度大幅上扬"]
    predict(model_path, vocab_path, input_strings)
